...
void GobangAI::aidraw(int x, int  y)
{
	...
	if (AIwriter.is_open())
	{
		AIwriter.seekp(int(AIturn * 9), ios::beg);
		if (x >= 10) { AIwriter << x; }
		else AIwriter << "0" << x;
		AIwriter << " ";
		if (y >= 10) { AIwriter << y; }
...
int GobangAI::LE(int x, int y, int flag, int role)
{
	for (int i = -1; i <= 1; i += 1)
	{
		for (int j = -1; j <= 1; j += 1)  // 八个方向 还可能是 3 ----  1 这样子
		{
			if (flag == 4)
			{
				if (0 <= x + 4 * i && x + 4 * i <= 14 && 0 <= y + 4 * j && y + 4 * j <= 14 && (i != 0 || j != 0))  // 八个方向检查    未过界限
				{
						...
						&& board[x + 3 * i][y + 3 * j] == board[x + 4 * i][y + 4 * j] && board[x + 4 * i][y + 4 * j] != 0)
					{
						...
						if (role == -1)
						{
							scoreboard[nextx][nexty] -= 3000*cline;
						}
					}
				}
			}
				...
				if (0 <= x + 3 * i && x + 3 * i <= 14 && 0 <= y + 3 * j && y + 3 * j <= 14 && (i != 0 || j != 0))
				{
						...
						&& board[x + 3 * i][y + 3 * j] != 0)
					{
						if (0 <= x + 4 * i && x + 4 * i <= 14 && 0 <= y + 4 * j && y + 4 * j <= 14 && 0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x + 4 * i][y + 4 * j] == 0 && board[x - i][y - i] == 0)   //             活  4         // 下一个只可以是 非staticpower  或者 0
						{
							if (role == 1)
							{
								if (staticpower == 2) { scoreboard[x][y] += 2000*cline; }  // 玩家活四后来处理
								if (staticpower == 1) { scoreboard[x][y] += 3000*cline; } // 自己活四优先处理
							}
							if (role == -1)
							{
								scoreboard[nextx][nexty] -= 1000*cline;
							}
						}
						else if ((0 <= x + 4 * i && x + 4 * i <= 14 && 0 <= y + 4 * j && y + 4 * j <= 14 && board[x + 4 * i][y + 4 * j] == 0) || (0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x - i][y - i] == 0)) // 死4
						{
							if (role == 1)
							{
								...
								{
									fourplayer_dying++;
									if (fourplayer_dying == 1) { scoreboard[x][y] += 666*cline; }
									else scoreboard[x][y] += 1334*cline;
								}  // 玩家死四后来处理
								if (staticpower == 1)
								{
									fourself_dying++;
									if (fourself_dying == 1) { scoreboard[x][y] += 1000 * cline; }
									else scoreboard[x][y] += 2000 * cline;
								} // 自己死四优先处理
							}
							//else // 被夹在中间   //没意义
							if (role == -1)
							{
								fourself_dying++;
								if (fourself_dying == 1) { scoreboard[nextx][nexty] -= 200 * cline; }
								else scoreboard[nextx][nexty] -= 1000 * cline;
							}
						}
					}
				}
			...
			if (flag == 2)
			{
					...
					board[x + 2 * i][y + 2 * j] != 0)
				{
					if (0 <= x + 3 * i && x + 3 * i <= 14 && 0 <= y + 3 * j && y + 3 * j <= 14 && 0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x + 3 * i][y + 3 * j] == 0 && board[x - i][y - i] == 0)  // 活3
					{
						if (role == 1)
						{
							if (staticpower == 2) { scoreboard[x][y] += 500 * cline; }  // 玩家活三后来处理
							if (staticpower == 1) { scoreboard[x][y] += 750 * cline; } // 自己活三优先处理
						}
						if (role == -1)
						{
							scoreboard[nextx][nexty] -= 500 * cline;
						}
					}
					if ((0 <= x + 3 * i && x + 3 * i <= 14 && 0 <= y + 3 * j && y + 3 * j <= 14 && board[x + 3 * i][y + 3 * j] == 0) || (0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x - i][y - i] == 0))
					{
						if (role == 1)
						{
							if (staticpower == 2) { scoreboard[x][y] += 84 * cline; }  // 玩家死三
							if (staticpower == 1) { scoreboard[x][y] += 125 * cline; } // 自己死三
						}
						if (role == -1)
						{
							scoreboard[nextx][nexty] -= 50 * cline;
						}
					}
				}
			}
			if (flag == 1)
			{
				if (board[x][y] == board[x + 1 * i][y + 1 * j] && board[x + 1 * i][y + 1 * j] != 0)
				{
					if (0 <= x + 2 * i && x + 2 * i <= 14 && 0 <= y + 2 * j && y + 2 * j <= 14 && 0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x + 2 * i][y + 2 * j] == 0 && board[x - i][y - i] == 0)  // 活3
					{
						if (role == 1)
						{
							if (staticpower == 2) { scoreboard[x][y] += 167 * cline; }  // 玩家活二后来处理
							if (staticpower == 1) { scoreboard[x][y] += 250 * cline; } // 自己活二优先处理
							//living ++;
						}
						if (role == -1)
						{
							scoreboard[nextx][nexty] -= 200 * cline;
						}
					}
					if ((0 <= x + 2 * i && x + 2 * i <= 14 && 0 <= y + 2 * j && y + 2 * j <= 14 && board[x + 2 * i][y + 2 * j] == 0) || (0 <= x - i && x - i <= 14 && 0 <= y - i && y - i <= 14 && board[x - i][y - i] == 0))
					{
						if (role == 1)
						{
							if (staticpower == 2) { scoreboard[x][y] += 40 * cline; }  // 玩家死二
							if (staticpower == 1) { scoreboard[x][y] += 60 * cline; } // 自己死二
						}
						if (role == -1)
						{
							scoreboard[nextx][nexty] -= 5 * cline;
						}
					}
				}
			}
			if (flag == 0)
			{
				if (board[7][7] == 0) { scoreboard[7][7] = 100 * cline; }
				...
				if (point_ymax - point_ymin >= flag)
				{
					...
					if (flag == 3)
					{
						if (point_ymin - 1 >= 0 && board[x][point_ymin - 1] == 0 && point_ymax + 1 <= 14 && board[x][point_ymax + 1] == 0)   // 未出界 // 且为空  // 活4
						{
							if (role == 1)
							{
								if (staticpower == 2) { scoreboard[x][y] += 2000 * cline; }
								if (staticpower == 1) { scoreboard[x][y] += 3000 * cline; }
							}
							if (role == -1)
							{
								scoreboard[nextx][nexty] -= 1000 * cline;
						...
						else if ((point_ymin - 1 >= 0 && board[x][point_ymin - 1] == 0) || (point_ymax + 1 <= 14 && board[x][point_ymax + 1] == 0))
						{
							if (role == 1)
							{
								...
								{
									fourself_dying++;
									if (fourself_dying == 1) { scoreboard[x][y] += 666 * cline; }
									else scoreboard[x][y] += 1334 * cline;
								}
								if (staticpower == 1)
								{
...
{
	if (Gofirst == 1 && AIturn == 0 && board[7][7] == 0) { aidraw(7, 7); }
	else
	{
		int xi = minx; int xa = maxx; int yi = miny; int ya = maxy;  int temp;
		if (xi > 0) xi--; if (xa < 14) xa++; if (yi > 0) yi--; if (ya < 14) ya++;  // 扩大一格
...
int GobangAI::withdraw()
{
	if (AIturn > 0)
	{
		AIturn--; int fake_AIturn = AIturn + 1; int x; int y; int Fpower;
...
int GobangAI::goforward()
{
	if (AIturn < maxAIturn)
	{
		AIturn++; int fake_AIturn = AIturn; int x; int y; int Fpower;
		//board[AIhist[2 * AIturn]][AIhist[2 * AIturn + 1]] = 0;
		ifstream AIreader("AIchessboard.txt");
		if (AIreader.is_open())
		{
...
void GobangAI::keepsame() { maxAIturn = AIturn; }